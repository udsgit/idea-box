<template>
  <article class="p-3 mb-4 rounded-lg sm:flex sm:items-center">
    ...
    <img
        v-if="userIdea"
        @click="removeIdea"
        class="mr-3 cursor cursor-pointer"
    ...
    <!-- Vote -->
    <section class="pt-3 border-t-2 mt-6 border-black
      ...
      <nav v-if="user && !userVoted" class="flex justify-center sm:block">
        <img @click="voteIdea(true)" class="w-10 cursor-pointer" src="@/assets/img/arrow.svg" alt="Vote up" />
        <img @click="voteIdea(false)" class="w-10 cursor-pointer transform rotate-180" src="@/assets/img/arrow.svg"
             alt="Vote down" />
      </nav>
    </section>
    <!-- End of Vote -->
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const removeIdea = () => emit("remove-idea", { name: props.idea.name, id: props.idea.id});
    return {
      voteIdea,
      userVoted,
      userIdea,
